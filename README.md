...
- **Heartbeat監視**: 60秒ごとに稼働状態を確認し、異常を即時検知
...
#### 8. Claudeサブスクリプション / APIキーの選択

AIエージェントをどの課金方式で動かすかを**Web設定画面から選択できます**。追加コードは不要です。

| モード | 説明 | 向いている用途 |
|--------|------|----------------|
| **Claudeサブスクリプション**（デフォルト） | Claude Proなどのサブスクプランで動作。従量課金なし | 個人・小チームでAPIコストをかけずに使いたい場合 |
| **Anthropic APIキー** | 入力したAPIキーで直接呼び出し。従量課金 | エンタープライズ利用・大量処理が必要な場合 |

設定変更は Web管理画面の **設定 → エージェント実行モード** から数クリックで完了します。

> **導入効果の目安**: 「まずサブスクで試して、本格導入時にAPIキーへ移行」という段階的な運用が設定変更だけで実現できます。
...
- [Node.js 18以上](https://nodejs.org/) — JavaScriptの実行環境（無料）
- [pnpm](https://pnpm.io/) — `npm install -g pnpm` でインストール（無料）
- [Docker](https://www.docker.com/) — データベースの起動に使用（無料）
...
# packages/api/.env を開いて API_KEY_SALT と ENCRYPTION_KEY を任意の文字列に変更

# 4. データベースを起動（1コマンドで完了）
docker compose up -d
...
**起動確認**: ブラウザで `http://localhost:3000/health` を開き、`{"status":"ok","database":"connected"}` と表示されれば成功です。
...
- **Heartbeat monitoring**: Status checked every 60 seconds; anomalies surfaced instantly
...
#### 8. Claude Subscription or API Key — Your Choice

Choose how AI agents are billed directly from the **Web settings page**. No code changes required.

| Mode | Description | Best for |
|------|-------------|----------|
| **Claude Subscription** (default) | Runs on your Claude Pro/Team plan — no per-request cost | Individuals and small teams who want to avoid usage-based billing |
| **Anthropic API Key** | Calls the Anthropic API directly with your key — pay-as-you-go | Enterprise workloads and high-volume processing |

Switch modes in the Web dashboard under **Settings → Agent Execution Mode** in a few clicks.

> **Expected impact**: Start with your existing Claude subscription, then move to an API key for production scale — with no code changes, just a settings update.
...
- [Node.js 18+](https://nodejs.org/) — Free runtime for JavaScript
- [pnpm](https://pnpm.io/) — `npm install -g pnpm` (free)
- [Docker](https://www.docker.com/) — For the database (free)
...
cp .env.example packages/api/.env
# Open packages/api/.env and set API_KEY_SALT and ENCRYPTION_KEY to any strings you choose

# 4. Start the database (one command)
docker compose up -d
...
**Verify**: Open `http://localhost:3000/health` — you should see `{"status":"ok","database":"connected"}`.
