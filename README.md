# maestro — AIスキルを「ちゃんと運用する」ためのプラットフォーム
...
やっていることはシンプルで、AI スキルの「起動・監視・停止・コスト管理」を自動化します。いわば **AI スキル版の運用管理ツール** です。
...
| API の請求が想定外に膨らむ | 月額の予算上限を設定しておけば、超えた瞬間にスキルを自動停止 |
| 誰が何を実行したか分からない | 全操作にタイムスタンプ付きのログが残る（監査対応にも使える） |
| タスクの割り振りが手作業 | タスクを登録すると、空いているスキルに自動でアサイン |
...
**何をするか：** 30秒ごとに、有効な全スキルに「生きてる？」と確認する。

**動きの流れ：**

1. データベースから `enabled: true` のスキル一覧を取得
2. 最大3並列でアダプター経由のヘルスチェックを実行
3. 応答があれば `last_heartbeat_at` を更新
4. 応答がなければ `agent_runtime_state` を `crashed` に設定（→ クラッシュ回復エンジンが拾う）
5. ついでに、保留中のスキル間引き継ぎ（handoff）も処理する

### 補足: スキル間引き継ぎ（handoff）とチェーン

ハートビートエンジンは、タスクが完了した際に「次のスキルへ引き継ぐ」処理も担当します。

- **1対1 handoff**: スキルAが完了 → スキルBへ成果物を渡して続きを依頼
- **チェーン（A→B→C）**: 複数スキルを順番につなげてパイプラインとして動かす

設計仕様は `docs/handoff/` および `docs/chain/` を参照してください。

### 2. クラッシュ回復エンジン（crash-recovery.ts）

**何をするか：** 60秒ごとに、クラッシュしたスキルを見つけて自動で復旧する。

**動きの流れ：**

1. `agent_runtime_state` テーブルから `status: crashed` のものを探す
2. 再起動回数が3回未満なら → 状態を `idle` に戻す（次のハートビートで再実行される）
3. 再起動回数が3回に達したら → スキルを無効化して停止（無限ループ防止）
...
`packages/cli/src/commands/` に17個のコマンドが実装されています（`backup` は `create` と `list` の2サブコマンドを持ちます）。
...
maestro は、Claude・Gemini・Codex などの AI スキルを業務で安全に運用するためのオープンソースプラットフォームです。30秒間隔のヘルスチェック、クラッシュ時の自動復旧（最大3回）、月額予算超過時の自動停止という3つのコアエンジンを中心に、タスクの自動割り当て、人間による承認ゲート、スキル間の引き継ぎ（handoff チェーン）、Webhook 連携、操作ログの全記録といった機能を提供します。マルチテナント設計のため複数の会社・チームでの共有利用にも対応しており、REST API・CLI（17コマンド）・Web ダッシュボードの3つのインターフェースから操作できます。
