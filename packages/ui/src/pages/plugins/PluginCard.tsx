...
/** 行が「使い方の例」として適切かチェック */
function isUsableExample(l: string): boolean {
  if (!l) return false;
  if (l.startsWith('#') || l.startsWith('|')) return false;
  if (l.endsWith(':')) return false;
  if (l.includes('**')) return false; // Markdownボールド
  if (/^\d+\.\s/.test(l)) return false; // 番号付きリスト
  if (l.length < 5 || l.length > 100) return false;
  return true;
}

/** サンプルプロンプトを usage_content から抽出
 * "When to Activate" / "When to Use" セクションの箇条書きを優先して返す */
export function extractSamplePrompts(usageContent: string | undefined, _name: string): string[] {
  ...
  // "When to Activate" / "When to Use" / "When to Invoke" 等のセクションを探す
  ...
  if (whenMatch) {
    const bullets = whenMatch[1]
      ...
      .filter((l) => isUsableExample(l));
    if (bullets.length > 0) {
      return bullets.slice(0, 3);
    }
  }

  // フォールバック: 箇条書き行を先頭から取得
...
export function PluginCard({
  ...
  // DB に保存済みの usage_examples を優先。未保存の場合はクライアント側で抽出
  const samplePrompts: string[] = plugin.usage_examples
    ? (() => { try { return JSON.parse(plugin.usage_examples) as string[]; } catch { return []; } })()
    : extractSamplePrompts(plugin.usage_content, plugin.name);
