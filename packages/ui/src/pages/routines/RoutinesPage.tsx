...
/**
 * cron式（分 時 日 月 曜日）から次回実行時刻を計算する簡易実装。
 * 標準的な5フィールドcronのみ対応。ワイルドカードと単一値のみ処理。
 */
function getNextRunAt(cron: string): Date | null {
  ...
  while (candidate <= limit) {
    // 月チェック (1-12)
    if (monthField !== '*') {
      const m = parseInt(monthField, 10);
      if (!isNaN(m) && candidate.getMonth() + 1 !== m) {
    ...
    // 曜日チェック (0=日)
    if (dowField !== '*') {
      const targetDows = dowField.split(',').map(d => parseInt(d, 10));
      if (!targetDows.includes(candidate.getDay())) {
    ...
    if (domField !== '*') {
      ...
      if (!isNaN(dom) && candidate.getDate() !== dom) {
        candidate.setDate(candidate.getDate() + 1);
        candidate.setHours(0, 0, 0, 0);
        continue;
      }
    ...
    if (hourField !== '*') {
      ...
      if (!isNaN(h) && candidate.getHours() !== h) {
        if (candidate.getHours() > h) {
          candidate.setDate(candidate.getDate() + 1);
          candidate.setHours(0, 0, 0, 0);
        } else {
    ...
    // 分チェック
    if (minField !== '*') {
      const mn = parseInt(minField, 10);
      if (!isNaN(mn) && candidate.getMinutes() !== mn) {
        if (candidate.getMinutes() > mn) {
          candidate.setHours(candidate.getHours() + 1, mn, 0, 0);
        } else {
          candidate.setMinutes(mn, 0, 0);
        }
        continue;
      }
    }
      ...
      <div className="p-5">
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0 flex-1">
            ...
            {/* 次回実行時刻 */}
            {nextRunAt && (
              <div className="mt-1 flex items-center gap-1.5 text-xs text-th-text-4">
                <Clock className="h-3 w-3" />
                <span>次回実行: {formatDate(nextRunAt.toISOString())}</span>
              </div>
            )}
