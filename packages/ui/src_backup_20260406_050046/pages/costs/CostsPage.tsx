  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="bg-th-surface-0 rounded-th-md p-6 border border-th-border">
        ...
        <p className="text-4xl font-bold text-th-accent">
          ${totalUsd.toFixed(4)}
        </p>
        <p className="text-xs text-th-text-4 mt-1">{t('costs.eventCount', { count: (events ?? []).length })}</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-th-surface-0 rounded-th-md p-6 border border-th-border">
          ...
          {(events ?? []).length > 0 ? (
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-th-border">
                  {(events ?? []).map((event) => (
                    <tr key={event.id} className="text-th-text-2">
                      ...
                      <td className="py-2 text-right font-bold text-th-danger">
            ...
            <p className="text-th-text-3">{t('costs.noEvents')}</p>
          )}
        </div>
        ...
        <div className="bg-th-surface-0 rounded-th-md p-6 border border-th-border">
          ...
          {policiesLoading ? (
            ...
            <div className="space-y-3">
              {(policies ?? []).map((policy) => (
                <div key={policy.id} className="bg-th-bg rounded p-4 border border-th-border">
                  <div className="flex justify-between items-center mb-2">
                    <span className="text-sm text-th-text-3">{t('costs.budgetLimit')}</span>
                    <span className="font-bold text-th-warning">
                      ${parseFloat(policy.limit_amount_usd).toFixed(2)}
                    </span>
                  </div>
                  <div className="flex justify-between items-center mb-2">
                    <span className="text-sm text-th-text-3">{t('costs.budgetPeriod')}</span>
                    <span className="text-sm">{policy.period}</span>
                  </div>
                  {policy.alert_threshold && (
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-th-text-3">{t('costs.alertThreshold')}</span>
                      <span className="text-sm text-th-warning">
            ...
            <p className="text-th-text-3">{t('costs.noBudgetPolicies')}</p>
          )}
        </div>
      </div>
    </div>
