...
export default function IssuesPage() {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      setCreateError((err as any)?.response?.data?.message ?? t('common.error'));
    }
  };
  ...
  return (
    <div className="p-6 space-y-6 max-w-5xl mx-auto">
      ...
      {showCreate && (
        <div className="bg-th-surface-0 rounded-th-md p-4 border border-th-border space-y-3">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="bg-th-accent hover:bg-th-accent/80 disabled:opacity-50 px-4 py-2 rounded-th-md text-sm font-medium"
            >
              {t('common.create')}
            </button>
            <button
              ...
              className="bg-th-surface-2 hover:bg-th-surface-1 px-4 py-2 rounded-th-md text-sm font-medium"
            >
              {t('common.cancel')}
            </button>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,1.4fr)_repeat(3,minmax(0,0.8fr))] gap-3">
        <input
          ...
          className="w-full rounded-th-md border border-th-border bg-th-surface-0 px-3 py-2 text-sm text-th-text"
        />
        <select
          value={statusFilter}
              ...
              <Card hoverable>
                <CardBody className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                  <div className="flex-1 min-w-0">
                    <p className="text-xs text-th-text-4 mb-0.5">{issue.identifier}</p>
                    <h3 className="font-semibold text-th-text group-hover:text-th-accent transition-colors truncate">
                      {issue.title}
                    </h3>
                    <div className="mt-2 flex flex-wrap items-center gap-2 text-xs text-th-text-4">
